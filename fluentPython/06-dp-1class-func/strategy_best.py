...
def best_promo(order):  # <2>与其他几个 *_promo 函数一样,best_promo 函数的参数是一个 Order 实例。
    """Select best discount available
    """
    return max(promo(order) for promo in promos)  # <3>使用生成器表达式把 order 传给 promos 列表中的各个函数,返回折扣额度最大的那个函数。
